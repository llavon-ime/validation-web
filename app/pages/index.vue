...
} from "#shared/utils/zhuyin";
import { computed, onMounted, ref, watch } from "vue";
import {
  acceptContributionAgreement,
  getContributionAgreement,
...
const agreementStatusReady = ref(false);
...
const showAgreement = computed(
  () => agreementStatusReady.value && agreementAcceptedAt.value === null,
);
...
onMounted(async () => {
  ...
  try {
    const response = await getContributionAgreement();
    agreementVersion.value = response.agreement.requiredVersion;
    agreementAcceptedAt.value = response.agreement.acceptedAt;
  } catch (error) {
    agreementError.value =
      error instanceof Error ? error.message : "無法讀取同意書狀態";
  } finally {
    agreementStatusReady.value = true;
  }
});
</script>
  ...
  <div class="site-shell">
    <header class="topbar">
      ...
      <div class="account-area">
        <a class="dataset-link" href="/dataset">瀏覽資料集</a>
        ...
        <a v-else class="github-button compact" href="/auth/github">
          使用 GitHub 登入
        </a>
      </div>
    </header>
